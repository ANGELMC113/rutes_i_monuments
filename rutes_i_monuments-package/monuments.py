...
METADATA_MONUMENTS_FILENAME = (
    MONUMENTS_FILENAME[:-4] + ".json"
...
def _get_point(soup: BeautifulSoup) -> str | None:
    ...
    pattern = re.compile(
        r"var destinations = \['([\d\.\s]+)'\];"
    )  # Pattern to search for the coordinates
...
def _download_monuments(start: tuple[int, int, int], feedback: bool = True) -> None:
    ...
    for i in range(x, len(TYPES)):  # for every type in TYPES
        ...
        for j in range(
            y, len(APPENDIX_TYPES[monument_type])
        ):  # for every class in this type
                        ...
                        try:
                            ...
                            if coords is not None:
                                text = (link.text).split(
                                    sep=" – "
                                )  # – U+2013                          # Maybe we split name and location

                                num_name_location = text[0].split(
                                    sep=". "
                                )  # Maybe location is still with name
                                num_in_subclass = num_name_location[
                                    0
                                ]  # Always the number, found before "."
                                name_and_location = num_name_location[1].split(
                                    sep=" / "
                                )  # Now we separate the location from the name if no "–" was found
                                name = name_and_location[
                                    0
                                ]  # Always the name, found after "." and before "–" or "/"

                                location: str = (
                                    ""  # Location is everything else we see after the name and before the coordinates
                                )
                                ...
                                with open(
                                    MONUMENTS_FILENAME, "a", encoding="utf8"
                                ) as file:
                                    file.write(line)
                                metadata = {
                                ...
                                with open(
                                    METADATA_MONUMENTS_FILENAME, "w"
                                ) as metadata_file:
                                    json.dump(metadata, metadata_file)
                                if feedback:
                                    print(
                                        f"Monument {link.text}, located at {coords} has been added."
                                    )

                            else:
                                if feedback:
                                    print(
                                        f"Monument {link.text} has not been added because its coordinates have not been found."
                                    )

                    except ConnectionResetError:
                        print(
                            f"Connection Reset Error. Waiting {CONNECTION_RETRY_DELAY} and trying again."
                        )
...
def _read_monuments_metadata() -> tuple[bool, tuple[int, int, int]]:
    try:
        ...
        return metadata["is_all_data_downloaded"], (
            metadata["type_index"],
            metadata["subclass_index"],
            metadata["place_index"],
        )
    except FileNotFoundError:
        raise FileNotFoundError(
            f"{METADATA_MONUMENTS_FILENAME} not found. To avoid having incorrect data, \
    ...
    if not os.path.isfile(MONUMENTS_FILENAME):  # File does not exist
        ...
        if is_all_data_downloaded:
            ...
            if feedback:
                ...
                next_monument = (
                    last_downloaded_monument[0],
                    last_downloaded_monument[1],
                    last_downloaded_monument[2] + 1,
                )
