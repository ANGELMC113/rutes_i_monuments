...
from generics import Point, Segment, Segments, check_file_extension, DEFAULT_FEEDBACK
...
class AttribStaticMap(StaticMap, object):
    """StaticMap subclass that adds an attribution label to the bottom-right corner."""
    # Based on a class published by Henner at GitHub (https://github.com/komoot/staticmap/issues/20)
    # Modified to fix errors, simplify code, and increase compatibility

    ATTRIBUTION = "Map base and paths (segments) data from\n© OpenStreetMap (openstreetmap.org/copyright)"
    ATTRIBUTION_FONT_SIZE = 24
    PADDING = 10
    OFFSET = 14
    ...
    def _draw_features(self, image):
        ...
        # Font
        try:
            font = ImageFont.truetype("FreeMono.ttf", self.ATTRIBUTION_FONT_SIZE)
        except OSError:
        ...
        offset = self.OFFSET
        x = image.size[0] - text_w - offset
        y = image.size[1] - text_h - offset

        # Background rectangle
        padding = self.PADDING
        draw.rectangle(
            [(x - padding, y - padding), (x + text_w + padding, y + text_h + padding)],
            fill=(255, 255, 255, 180), # Light white
        )

        draw.multiline_text((x, y), self.ATTRIBUTION, font=font, fill="black")
...
def _check_parameters(colors: dict[str, str], sizes: dict[str, int]) -> None:
    ...
    if missing_color_keys or missing_size_keys:
        raise KeyError(
            f"There are some parameters missing in the given dictionaries. \
            Check the documentation to ensure you have properly defined the parameters."
        )


def _map_from_segments(
    data: Segments | tuple[Segments, list[Point]],
    map: AttribStaticMap,
    colors: dict[str, str],
    sizes: dict[str, int],
) -> None:
...
def _map_from_graph(
    G: Graph, map: AttribStaticMap, colors: dict[str, str], sizes: dict[str, int]
) -> None:
...
def export_png_map(
    ...
    try:
        if feedback:
            print("Creating map.")
        map = AttribStaticMap(sizes["map_width"], sizes["map_height"])
