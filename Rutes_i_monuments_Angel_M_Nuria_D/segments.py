...
import gpxpy
import os.path
from generics import MONUMENTS_FILENAME, Point, Segment, Segments, Box, check_file_extension, distance
...
def _download_segments(filename: str, box_str: str, startpage: int = 0, endpage: int = -1, feedback: bool = True) -> None:
    ...
    while endpage == -1 or page <= endpage:
        ...
        try:
            ...
            with open(metadata_filename, "w") as metadata:
                json.dump({"box_str": box_str, "is_all_data_gathered": False, "last_page": page}, metadata)
            if feedback: 
    ...
    if page > 0:
        with open(metadata_filename, "w") as metadata:
            json.dump({"box_str": box_str, "is_all_data_gathered": True, "last_page": page - 1}, metadata)        
        if feedback:
...
def _segment_generator(filename: str, max_distance_meters: int = SEGMENT_MAX_LENGHT_METERS) -> Iterator[Segment]:
...
def get_segments(filename: str, box: Optional[Box] = None, endpage: int = -1, feedback: bool = True) -> Segments:
    ...
    if filename == MONUMENTS_FILENAME:
        raise FileExistsError(f"Please, do not name any file {MONUMENTS_FILENAME}, as this filename is reserved to monument data gathering. Change filename and try again.")
    ...
    if not os.path.isfile(filename): # File does not exist
        ...
        try:
            file_box_str, is_all_data_downloaded, last_downloaded_page = _read_metadata(filename)
        except FileNotFoundError:
            metadata_filename = filename[:-4]+".json"
            raise FileNotFoundError(f"{metadata_filename} not found. To avoid having incorrect data, \
