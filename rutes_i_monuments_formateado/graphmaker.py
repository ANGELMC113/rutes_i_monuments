...
def _nodes_with_coordinates(
    centroid_coords: list[Point],
) -> list[tuple[int, dict[str, Point]]]:
...
def _make_graph_from_kmeans(
    centroid_coords: list[Point], edges: list[tuple[int, int]]
) -> Graph:
...
def _simplified_graph(G: Graph, epsilon: float) -> Graph:
    ...
    while node_q:
        ...
        if node in G.nodes:
            ...
            if (
                len(neighbors) == 2
            ):  # If a node more than three nodes we can't lose it. We only have to analize once each node. # type: ignore
