...
from generics import Point, Box, distance, DEFAULT_FEEDBACK
...
def _filter_monuments(box: Box) -> Monuments:
    ...
    return [
        mon
        for mon in get_monuments()
        if (
            box.bottom_left.lat < mon.location.lat < box.top_right.lat
            and box.bottom_left.lon < mon.location.lon < box.top_right.lon
        )
    ]
...
def _approximate_and_add_monuments_to_nodes(
    G: Graph, endpoints: Monuments
) -> dict[int, list[str]]:
...
def _find_shortest_paths(
    G: Graph, source: int, targets: list[int], feedback: bool = DEFAULT_FEEDBACK
) -> list[list[int]]:
    ...
    for node in targets:
        try:
            ...
            if feedback:
                print(
                    f"No path found from the starting node to node {node}. Some monuments are not reachable."
                )
    return shortest_paths


def _create_tree(
    G: Graph, start: Point, endpoints: Monuments, feedback: bool = DEFAULT_FEEDBACK
) -> Graph:
    ...
    if len(nodes_with_monuments) == 0:
        raise ValueError(
            "There seem no be no monuments inside this Box. Program has stopped."
        )
...
def find_routes(
    G: Graph, box: Box, start: Point, feedback: bool = DEFAULT_FEEDBACK
) -> Graph:
    ...
    if feedback:
        ...
        print(
            f"These are the monuments and their locations (in latitude - longitude format):"
        )
        for monument in monuments_in_box:
            print(
                f"{monument.name} ({monument.sublcass}) at {monument.location.lat}, {monument.location.lon}"
            )
