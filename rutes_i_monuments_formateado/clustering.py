...
def _kmeans_centroids(
    segments: Segments, n_clusters: int
) -> "tuple[ndarray[int], list[Point]]":
...
def cluster(
    segments: Segments, n_clusters: int
) -> "tuple[ndarray[int], list[Point], list[tuple[int, int]], Segments]":
    ...
    for _ in segments:
        if (
            centroid_labels[i] != centroid_labels[i + 1]
        ):  # If segments joins different cluster
            edge = (
                centroid_labels[i],
                centroid_labels[i + 1],
            )  # Edge between centroids (as ints)
            new_segment = Segment(
                centroid_coords[centroid_labels[i]],
                centroid_coords[centroid_labels[i + 1]],
            )  # Segment between centroids (as Points)
